    ...
    >
      ...
      <div className="relative mx-auto max-w-[1400px] px-4 py-6 lg:px-8">
        <header className="mb-4 rounded-2xl border border-white/20 bg-white/85 p-4 shadow-xl backdrop-blur md:p-5">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              <button type="button" className="rounded-lg bg-slate-900 px-4 py-2 text-sm font-medium text-white hover:bg-slate-700" onClick={onBack}>
                Back to Dashboard
              </button>
            </div>
          </div>
        ...
        <div className="grid gap-4 xl:grid-cols-[260px_1fr]">
          <aside className="rounded-2xl border border-cyan-200/30 bg-slate-900/80 p-4 text-white shadow-2xl backdrop-blur">
              ...
              {navItems.map((item) => (
                <button
                  ...
                  className={`w-full rounded-lg px-3 py-2 text-left text-sm transition ${
                    activeView === item.id ? 'bg-cyan-400 text-slate-900 font-semibold' : 'bg-white/10 text-slate-100 hover:bg-white/20'
                  }`}
                >
                  {item.label}
                </button>
                    ...
                    <div>
                      ...
                      <div className="mt-2 flex flex-wrap gap-2">
                        {selectedStaff.contact && selectedStaff.contact !== 'N/A' ? (
                          <a href={`tel:${selectedStaff.contact}`} className="rounded-lg bg-slate-900 px-3 py-1.5 text-xs font-semibold text-white hover:bg-slate-700">
