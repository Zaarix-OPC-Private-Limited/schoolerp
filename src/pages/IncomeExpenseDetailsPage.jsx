...
function IncomeExpenseDetailsPage() {
  ...
  const { incomeLedger: ctxIncomeRows, expenseLedger: ctxExpenseRows, setIncomeLedger: ctxSetIncome, setExpenseLedger: ctxSetExpense } = useAppContext()
  ...
  const incomeLedger = ctxIncomeRows ?? localIncomeLedger
  const expenseLedger = ctxExpenseRows ?? localExpenseLedger
  const setIncomeLedger = ctxSetIncome ?? setLocalIncomeLedger
  const setExpenseLedger = ctxSetExpense ?? setLocalExpenseLedger
      ...
      <div className="mx-auto w-full max-w-7xl px-5 py-6 sm:px-8">
        <header className="mb-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <button type="button" className="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50" onClick={() => navigate(-1)}>
              ← Back
            </button>
          </div>
        </header>
