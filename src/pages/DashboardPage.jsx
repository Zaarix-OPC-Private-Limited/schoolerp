...
function DashboardPage() {
  ...
  const { studentRecords: students = [], teacherRecords: teachers = [], incomeLedger = defaultIncomeLedger, expenseLedger = defaultExpenseLedger, loggedInDisplayName: welcomeName = 'Principal', loggedInRole: userRole = '', loggedInEmail: userEmail = '', handleLogout } = useAppContext()

  const onAddStudent = () => navigate('/students/add')
  const onAddTeacher = () => navigate('/teachers/add')
  const onOpenStudents = () => navigate('/students')
  const onOpenMarksheet = () => navigate('/marksheet')
  const onOpenFees = () => navigate('/fees')
  const onOpenTeacherHub = () => navigate('/teachers')
  const onOpenStaffHub = () => navigate('/staff')
  const onOpenAttendance = () => navigate('/attendance/students')
  const onOpenTeacherAttendance = () => navigate('/attendance/teachers')
  const onOpenTransport = () => { }
  const onOpenReports = () => { }
  const onOpenIncomeExpenseDetails = () => navigate('/finance')
  const onOpenSettings = () => { }
  const onOpenGuide = () => navigate('/guide')
  const onOpenNotice = () => navigate('/notices')
                ...
                {visibleLeaveRequests.map((item) => (
                  <article key={item.id} className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-2">
                    <div className="mb-1 flex items-center justify-between gap-2">
                      ...
                      <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase ${item.status === 'approved'
