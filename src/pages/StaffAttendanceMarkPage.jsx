...
export default function StaffAttendanceMarkPage() {
    ...
    const { staffRecords = [] } = useAppContext()
    ...
    const peopleList = useMemo(() => {
        ...
        return staffRecords
            ...
            .filter((p) => !q || p.name.toLowerCase().includes(q) || p.designation.toLowerCase().includes(q) || p.dept.toLowerCase().includes(q))
    }, [staffRecords, searchText])
    ...
    useEffect(() => {
        ...
        const load = async () => {
            ...
            setStatusMap({})
            try {
                const res = await getAttendanceExceptions(selectedDate, 'STAFF')
                if (cancelled) return
                const map = {}
    ...
    const handleSubmit = useCallback(async () => {
        ...
        try {
            ...
            showToast('success', exCount === 0
                ? `All ${peopleList.length} staff marked Present — nothing saved.`
    ...
    return (
        <motion.section key="staff-att-mark" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}
            ...
            <div className="mx-auto max-w-5xl px-4 py-6 sm:px-8">
                ...
                <header className="mb-5 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                    <div className="flex flex-wrap items-center justify-between gap-3">
                        <div>
                            <p className="text-[10px] font-bold uppercase tracking-[0.25em] text-slate-400">Attendance · Staff</p>
                            <h1 className="text-xl font-bold text-slate-900">Staff Attendance</h1>
                            <p className="mt-0.5 text-xs text-slate-500">
                                All staff are <span className="font-semibold text-emerald-600">Present</span> by default.
                ...
                <div className="mb-4 flex flex-wrap items-center gap-3">
                    ...
                    <input type="text" placeholder="Search by name, department…" value={searchText}
                ...
                <div className="rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                    <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3">
                        <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">
                            {isLoading ? 'Loading…' : `${peopleList.length} Staff Member(s)`}
                        </p>
                    ...
                    {isLoading ? (
                        ...
                        <p className="py-12 text-center text-sm text-slate-500">
                            {searchText ? 'No results found.' : 'No staff found. Add staff members first.'}
                        </p>
                    ) : (
                                ...
                                return (
                                    <li key={person.id} className="flex flex-wrap items-center gap-3 px-4 py-3 hover:bg-slate-50 transition">
                                        {person.photo
                                            ...
                                            : <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-cyan-700 text-xs font-bold text-white">{getInitials(person.name)}</span>
                                        }
                ...
                <div className="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3 shadow-sm">
                    ...
                    <button type="button" disabled={isSubmitting || isLoading} onClick={handleSubmit}
                        className="rounded-lg bg-cyan-700 px-5 py-2 text-sm font-semibold text-white hover:bg-cyan-800 disabled:opacity-50 transition">
